...
impl Format {
    ...
    #[must_use]
    pub fn round_seconds(seconds: i64, time_format: TimeFormat) -> i64 {
        match time_format {
    ...
    #[must_use]
    pub fn seconds_to_duration(seconds: i64, time_format: TimeFormat) -> String {
        let seconds = Self::round_seconds(seconds, time_format);

        match time_format {
            TimeFormat::HoursMinutesSeconds => {
                let (h, m, s) = Self::seconds_to_hms(seconds);
                format!("{h:02}:{m:02}:{s:02}")
            }
            TimeFormat::HoursMinutes => {
                let (h, m, _) = Self::seconds_to_hms(seconds);
                format!("{h:02}:{m:02}")
            }
            TimeFormat::DecimalHours => {
                #[allow(clippy::cast_precision_loss)]
    ...
    pub fn string_to_seconds(text: &str, time_format: TimeFormat) -> Result<i64, String> {
        ...
        match time_format {
            ...
            TimeFormat::HoursMinutesSeconds => {
                ...
                let (h, m, s) = Self::parse_hms(&parts)?;
...
mod tests {
    ...
    mod seconds_to_hms {
        ...
        #[test]
        fn zero() {
            assert_eq!(Format::seconds_to_hms(0), (0, 0, 0));
        }

        #[test]
        fn normal() {
            // 3661 seconds = 1 hour, 1 minute, 1 second
            assert_eq!(Format::seconds_to_hms(3661), (1, 1, 1));
        }
    }
    ...
    mod round_seconds {
        ...
        fn hms_no_change() {
            assert_eq!(
                Format::round_seconds(3661, TimeFormat::HoursMinutesSeconds),
                3661
            );
        }
        ...
        fn seconds_no_change() {
            assert_eq!(Format::round_seconds(3661, TimeFormat::Seconds), 3661);
        }
        ...
        mod hours_minutes {
            ...
            fn rounds_to_nearest_minute() {
                ...
                assert_eq!(Format::round_seconds(89, TimeFormat::HoursMinutes), 60);

                // 91 sec → 1.52 min → 2 min
                assert_eq!(Format::round_seconds(91, TimeFormat::HoursMinutes), 120);
            }

            #[test]
            fn rounds_small_non_zero_up() {
                // Non-zero durations never become 00:00
                assert_eq!(Format::round_seconds(1, TimeFormat::HoursMinutes), 60);

                assert_eq!(Format::round_seconds(29, TimeFormat::HoursMinutes), 60);
            }
        }
        ...
        mod decimal_hours {
            ...
            fn rounds_to_quarters() {
                ...
                assert_eq!(Format::round_seconds(3600, TimeFormat::DecimalHours), 3600);

                // 4500 sec = 1.25h exact quarter
                assert_eq!(Format::round_seconds(4500, TimeFormat::DecimalHours), 4500);

                // rounding up
                // 5000 sec ≈ 1.39h → 1.50h
                assert_eq!(Format::round_seconds(5000, TimeFormat::DecimalHours), 5400);

                // rounding down
                // 3900 sec = 1.083h → 1.00h
                assert_eq!(Format::round_seconds(3900, TimeFormat::DecimalHours), 3600);
            }

            #[test]
            fn rounds_small_non_zero_up() {
                // Never show 0.00 hours for non-zero input
                assert_eq!(Format::round_seconds(1, TimeFormat::DecimalHours), 900);

                assert_eq!(Format::round_seconds(300, TimeFormat::DecimalHours), 900);
            }
        }
    }
    ...
    mod seconds_to_duration {
        ...
        mod hours_minutes {
            ...
            fn formats() {
                ...
                assert_eq!(
                    Format::seconds_to_duration(3661, TimeFormat::HoursMinutes),
                    "01:01"
                );

                // Small non-zero → normalized to 1 minute
                assert_eq!(
                    Format::seconds_to_duration(1, TimeFormat::HoursMinutes),
                    "00:01"
                );
            }
        }
        ...
        mod decimal_hours {
            ...
            fn formats() {
                ...
                assert_eq!(
                    Format::seconds_to_duration(300, TimeFormat::DecimalHours),
                    "00.25"
                );

                // 5000 sec → 83 min 20 sec → 1.388h → rounded to nearest quarter (1.50h)
                assert_eq!(
                    Format::seconds_to_duration(5000, TimeFormat::DecimalHours),
                    "01.50"
                );

                assert_eq!(
                    Format::seconds_to_duration(3600, TimeFormat::DecimalHours),
                    "01.00"
                );
            }
        }
        ...
        fn seconds_formats() {
            assert_eq!(
                Format::seconds_to_duration(3661, TimeFormat::Seconds),
                "3661"
            );
        }
        ...
        fn hours_minutes_seconds_formats() {
            assert_eq!(
                Format::seconds_to_duration(3661, TimeFormat::HoursMinutesSeconds),
                "01:01:01"
            );
        }
    }
    ...
    mod string_to_seconds {
        ...
        fn assert_invalid_input(text: &str, expected_error: &str, time_format: TimeFormat) {
            let error = Format::string_to_seconds(text, time_format).unwrap_err();
            assert_eq!(expected_error, error);
        }

        fn assert_valid_input(text: &str, time_format: TimeFormat) {
            let result = Format::string_to_seconds(text, time_format);
            assert!(result.is_ok());
        }
        ...
        mod seconds {
            use crate::core::format::tests::string_to_seconds::{
                assert_invalid_input, assert_valid_input,
            };
            use crate::core::time_format::TimeFormat;
